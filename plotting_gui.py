...
def create_3d_plot(results_info, coords_file, connections_file, top_n_modules):
    ...
    if os.path.exists(connections_file):
        try:
            ...
            # Infer neurotransmitters from combo name
            selected_nts = infer_neurotransmitters_from_combo(combo_name)
            
            # Filter connections by neurotransmitter type if column exists
            if 'nt_type' in conn_df.columns:
                conn_df = conn_df[conn_df["nt_type"].isin(selected_nts)].copy()
            
            conn_df["pre_root_id"] = conn_df["pre_root_id"].astype(str)
            conn_df["post_root_id"] = conn_df["post_root_id"].astype(str)

            # Only consider connections involving neurons in our dataset
            ...
            # Count degree (in + out connections)
            degree_counts = (
                pd.concat([
            ...
            top_merged["degree"] = top_merged["degree"].fillna(0)
            
        except Exception:
            top_merged["degree"] = 1.0
    else:
        top_merged["degree"] = 1.0
    ...
    for i, module in enumerate(sorted(top_merged["top_module"].unique())):
        ...
        if top_merged["degree"].max() > 0:
            sizes = cluster["degree"] / top_merged["degree"].max() * 15 + 3
        else:
            sizes = [8] * len(cluster)
...
def create_plotting_gui(coords_file="coordinates.csv", connections_file="connections_princeton.csv"):
    ...
    def update_analysis_info(change=None):
        if threshold_dropdown.value:
            ...
            try:
                ...
                <b>⚠️  Could not read analysis info:</b><br>
                {str(e)}
                </div>
                """
    ...
    def on_plot_clicked(b):
        ...
        if not threshold_dropdown.value:
            with output:
                print("⚠️  Please select a neurotransmitter combination and threshold.")
            return
        ...
        with output:
            print(f"🎨 Generating 3D plot...")
            print(f"📊 Analysis: {combo_dropdown.value}")
        ...
        try:
            ...
            with output:
                ...
                print(f"📈 Plotted {num_neurons:,} neurons across {num_modules_plotted} modules")
                print("🖱️  Attempting to display plot...")
            
            # Try to display the plot
            success = display_plot_safely(fig, output)
            
            if success:
                with output:
                    print("💡 Hover over points to see neuron details")
                    print("🔄 Use mouse to rotate, zoom, and explore the 3D visualization")
    ...
    main_layout = VBox([
        ...
        HTML("<h3 style='color: #555; margin: 15px 0 5px 0;'>🎛️ Plot Parameters</h3>"),
        modules_slider,
...
def quick_plot_latest(coords_file="coordinates.csv", connections_file="connections_princeton.csv", top_n=15):
    """
    Quick plot of the most recent analysis results without GUI.
    ...
    try:
        available_results = scan_available_results()
        if not available_results:
            print("❌ No Infomap results found. Run analysis GUI first.")
            return None
        ...
        for combo_name, results_list in available_results.items():
            for result in results_list:
                if result['mod_time'] > latest_time:
                    latest_time = result['mod_time']
                    latest_result = result
        
        if latest_result:
            print(f"🎨 Plotting latest result: {latest_result['combo_name']} (threshold {latest_result['threshold']})")
            fig, num_neurons, num_modules = create_3d_plot(
                latest_result, coords_file, connections_file, top_n
            )
            print(f"✅ Plot created with {num_neurons:,} neurons across {num_modules} modules")
            
            # Try to display
            try:
                fig.show()
                print("🖱️  Use mouse to interact with the 3D plot")
                return fig
            except Exception as